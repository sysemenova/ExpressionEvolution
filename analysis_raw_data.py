...
def partial(data, x1, x2, x3):
    ...
    # Note: we're controlling for x3
    print(len(data[x1]), end = " ")
    r123 = (r12-r13*r23)/np.sqrt((1-r13*r13)*(1-r23*r23))
    t = r123*(((len(data[x1])-2-1)/(1-r123*r123))**0.5)
    print("t", t, end = " ")
    p = stats.t.sf(np.abs(t), len(data[x1])-2-1)*2
    print("p", p)
    return r123, p
                        ...
                        if in_list[1].lower() == "all":
                            ...
                            for cur in expression_columns:
                                ...
                                for acc in expression_columns:
                                    if acc != cur:
                                        part, pval = partial(full_data, evo_rate_name, cur, acc)
                                        temp.append([part, pval])
                                    else:
                                        temp.append([0, 0])
                                partls[cur] = temp
                            print(partls)
                            ...
                            for i in expression_columns:
                                if i != in_list[1]:
                                    part, pval = partial(full_data, evo_rate_name, in_list[1], i)
                                    print("   %-23s %1.4f %1.4" %(i+":", part, pval))
                        else:
